...
folder = '../../'
### pump
E_probe_plus_A = np.genfromtxt("../../field.dat")
J_probe_plus_A = np.genfromtxt("../../equalt.dat")

E_A = np.genfromtxt("../../noProbe/field.dat")
J_A = np.genfromtxt("../../noProbe/equalt.dat")

######### Controls ####
probe_center = 20.0     ## Center of the pump or pulse.
time_length = 200.0 ## Fixed length of FT.
interpolation_times = 1 ## Interpolate raw data.
pad_zeros = 0   ## Add this many zeros at the end.
...
index = '000'
x0 = E_probe_plus_A[:,0] - probe_center
y0 = E_probe_plus_A[:,field_column] - E_A[:, field_column]

x1 = J_probe_plus_A[:,0] - probe_center
y1 = J_probe_plus_A[:,1] - J_A[:, 1]
